...
class Mouse extends FlxBasic
{
	...
	{
		if (FlxG.mouse.justPressed && hovered == null && !suspended)
		{
			System.signals.newInput.dispatch(EmptyMouseClick);
		}
	}
...
class Keys extends FlxBasic
{
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.BACKSPACE && !suspended)
		{
			System.signals.newInput.dispatch(KeyPressed(Backspace));
		}

		if (FlxG.keys.justPressed.UP && !suspended)
		{
			System.signals.newInput.dispatch(KeyPressed(Up));
		}

		if (FlxG.keys.justPressed.RIGHT && !suspended)
		{
			System.signals.newInput.dispatch(KeyPressed(Right));
		}

		if (FlxG.keys.justPressed.DOWN && !suspended)
		{
			System.signals.newInput.dispatch(KeyPressed(Down));
		}

		if (FlxG.keys.justPressed.LEFT && !suspended)
		{
